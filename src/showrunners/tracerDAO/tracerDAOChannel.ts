...
export default class TracerDAOChannel {
  // end_lte:${(Date.now() / 1000).toFixed()},
  // end_gte:${(Date.now() / 1000 - 21600).toFixed()}
  ...
  NEW_PROPOSAL_QUERY = gql`
    {
      proposals(
          ...
          where: { 
              space_in: ["tracer.eth"],
              start_lte:${(Date.now() / 1000).toFixed()},
              start_gte:${(Date.now() / 1000 - 21600).toFixed()} 
  ...
  public async fetchRecentlyEndedProposals() {
    ...
    return request(this.URL_SPACE_PROPOSAL, this.ENDED_PROPOSAL_QUERY)
  ...
  public async fetchVotesForFinsihedProposal(simulate) {
    ...
    for (let i = 0; i < finished.data.length; i++) {
      ...
      for (let j = 0; j < voteResult.votes.length; j++) {
        if (voteResult.votes[j].choice == 1) choice1++;
        else choice2++;
      }
      if (choice1 > choice2) res = Math.floor((choice1 / totalVotes) * 100);
      else res = Math.floor((choice2 / totalVotes) * 100);
      let message = {
  ...
  public async fetchNewProposalDetails() {
    ...
    return request(this.URL_SPACE_PROPOSAL, this.NEW_PROPOSAL_QUERY)
      .then(function(response) {
        return {
          success: true,
          data: response.proposals,
    ...
    switch (notificationType) {
      case NOTIFICATION_TYPE.NEW:
        ...
        for (let i = 0; i < proposal.length; i++) {
          ...
          message = `Title:${proposal[i].title}\nStart Date:${moment(proposal[i].start * 1000).format(
            'MMMM Do YYYY',
          )}\nEnd Date:${moment(proposal[i].end * 1000).format('MMMM Do YYYY')}`;
          payloadTitle = `New Proposal is live in TracerDAO`;
          payloadMsg = `[d:Title] : ${proposal[i].title}\n[s:Start Date] : ${moment(proposal[i].start * 1000).format(
            'MMMM Do YYYY',
          )}\n[t:End Date] : ${moment(proposal[i].end * 1000).format('MMMM Do YYYY')} [timestamp: ${Math.floor(
            new Date() / 1000,
          )}]`;
          notifType = 3;
      ...
      case NOTIFICATION_TYPE.STATUS:
        ...
        payloadMsg = `[d:Title] : ${proposal.title}\nChoice [s:${proposal.res}] got majority vote of [b:${
          proposal.percent
