...
export default class UniswapV3Channel {
    ...
    public async sendNotification(subscriber, title, body, payloadTitle, payloadMsg, notificationType, cta, sdk, simulate) {
        ...
        const epns:any = this.getEPNSInteractableContract(config.web3RopstenNetwork,sdk);
        ...
        const txn = await sdk.advanced.uploadToIPFS(payload, logger, simulate)
            .then(async (ipfshash) => {
                debugLogger("Success --> uploadToIPFS(): %o" + ipfshash);
                const storageType = 1; // IPFS Storage Type
                const txConfirmWait = 0; // Wait for 0 tx confirmation
                // Send Notification
                const notification = await sdk.advanced.sendNotification(
    ...
    public async getEPNSInteractableContract(web3network,sdk) {
        // Get Contract
        const walletKey = await this.getWalletKey()
